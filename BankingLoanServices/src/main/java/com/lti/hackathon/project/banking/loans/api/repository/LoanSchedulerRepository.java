...
public interface LoanSchedulerRepository extends CrudRepository<LoanScheduler, Long> {
	 ...
	 @Query(value = "select  p.name, sum(a.loan_balance - s.loan_balance) overdue from loan_scheduler s\r\n" + 
	 		...
	 		"INNER JOIN products p ON a.product_id=p.id where s.loan_contract_id=lc.id and  lc.account_id=a.id and a.product_id=p.id and s.repayment_date\r\n" + 
	...
	public List<Object[]> getLoanOverdueAmount(Date currentDate);
}
